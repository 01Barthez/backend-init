...
import type { UserJwtPayload } from '@/core/interface/global';
import { readFileSync } from '@/utils/fsUtils';

const userToken = {
  accessToken: (payload: UserJwtPayload): string => {
  ...
  refreshToken: (payload: UserJwtPayload): string => {
    const signOption: jwt.SignOptions = {
      algorithm: envs.JWT_ALGORITHM as jwt.Algorithm,
  ...
  verifyRefreshToken: (refreshToken: string) => {
    try {
      return jwt.verify(refreshToken, readFileSync(envs.JWT_REFRESH_PUBLIC_KEY_PATH)) as UserJwtPayload;
    } catch (error) {
