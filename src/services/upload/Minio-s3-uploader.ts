...
// Optional imports (commented-out to avoid hard runtime dependency if not installed)
  ...
  async scan(streamOrBuffer: Readable | Buffer): Promise<ScanResult> {
    ...
    try {
      ...
      return await new Promise<ScanResult>((resolve, reject) => {
        ...
        const input = streamOrBuffer instanceof Buffer ? Readable.from([streamOrBuffer]) : streamOrBuffer;
...
export class MinioUploader extends EventEmitter {
  ...
  private getPolicy(profile?: string): ValidationPolicy {
    if (profile && this.profiles[profile]) return { ...this.defaultPolicy, ...this.profiles[profile] };
  ...
  validate(meta: FileMeta, profile?: string) {
    ...
    const detectedMime = meta.contentType ?? mimeLookup(meta.filename) || 'application/octet-stream';
    ...
    if (policy.allowedExtensions && policy.allowedExtensions.length > 0) {
      if (!policy.allowedExtensions.includes(ext)) throw new ValidationError('extension_not_allowed', { ext });
    }
  ...
  async uploadBuffer(buffer: Buffer, meta: FileMeta, opts?: { profile?: string; prefix?: string; datePartition?: boolean; transform?: (buf: Buffer) => Promise<Buffer> }): Promise<UploadResult> {
    ...
    const key = this.buildKey(meta.filename, { prefix: opts?.prefix, datePartition: opts?.datePartition });
  ...
  async uploadStream(stream: Readable, meta: FileMeta, opts?: { profile?: string; prefix?: string; datePartition?: boolean; transformStream?: (s: Readable) => Readable }): Promise<UploadResult> {
    this.validate(meta, opts?.profile);
    ...
    if (this.scanner) {
      ...
      return this.uploadStreamNoScan(uploadStream, { ...meta, size: buffer.length }, { ...opts, keyOverride: key });
    }
  ...
  private async uploadStreamNoScan(stream: Readable, meta: FileMeta, opts?: { keyOverride?: string; transformStream?: (s: Readable) => Readable }): Promise<UploadResult> {
    const key = opts?.keyOverride ?? this.buildKey(meta.filename, { prefix: opts?.keyOverride ? undefined : undefined });

    const finalStream = opts?.transformStream ? opts.transformStream(stream) : stream;

    return this.retry(async () => {
      await this.ensureBucketExists();
      await this.client.putObject(this.bucket, key, finalStream, meta.size ?? undefined, { 'Content-Type': meta.contentType ?? 'application/octet-stream' });
  ...
  async createMultipartUpload(key: string, contentType?: string): Promise<string> {
    ...
    if (typeof this.client.createMultipartUpload === 'function') {
      const res = await this.client.createMultipartUpload({ Bucket: this.bucket, Key: key, ContentType: contentType });
   ...
   */
  async uploadPart(key: string, uploadId: string, partNumber: number, body: Buffer | Readable): Promise<{ ETag: string; PartNumber: number }> {
    if (typeof this.client.uploadPart === 'function') {
      // S3-like clients
      const params = { Bucket: this.bucket, Key: key, PartNumber: partNumber, UploadId: uploadId, Body: body };
  ...
  async completeMultipartUpload(key: string, uploadId: string, parts: Array<{ ETag: string; PartNumber: number }>) {
    if (typeof this.client.completeMultipartUpload === 'function') {
      const params = { Bucket: this.bucket, Key: key, UploadId: uploadId, MultipartUpload: { Parts: parts.map((p) => ({ ETag: p.ETag, PartNumber: p.PartNumber })) } };
  ...
  async abortMultipartUpload(key: string, uploadId: string) {
    if (typeof this.client.abortMultipartUpload === 'function') {
      return await this.client.abortMultipartUpload({ Bucket: this.bucket, Key: key, UploadId: uploadId });
    }
   ...
   */
  async multipartUploadManual(stream: Readable, meta: FileMeta, opts?: { profile?: string; prefix?: string; datePartition?: boolean; resume?: { uploadId: string; parts: Array<{ PartNumber: number; ETag: string }> } }) {
    this.validate(meta, opts?.profile);
    ...
    // create upload
    const uploadId = opts?.resume?.uploadId ?? (await this.createMultipartUpload(key, meta.contentType));

    // generator that yields parts from stream
    const partSize = this.multipartPartSize;
    const parts: Array<{ PartNumber: number; ETag?: string }> = opts?.resume?.parts?.map((p) => ({ PartNumber: p.PartNumber, ETag: p.ETag })) ?? [];

    let partNumber = 1;
    let buffer = Buffer.alloc(0);
    const uploadedParts: Array<{ ETag: string; PartNumber: number }> = opts?.resume?.parts?.map((p) => ({ ETag: p.ETag, PartNumber: p.PartNumber })) ?? [];
    ...
    // complete
    await this.completeMultipartUpload(key, uploadId, uploadedParts.map((p) => ({ ETag: p.ETag, PartNumber: p.PartNumber })));
  ...
  async presignedPutUrl(filename: string, expiresSeconds = 60 * 5, opts?: { prefix?: string; datePartition?: boolean }) {
    const key = this.buildKey(filename, { prefix: opts?.prefix, datePartition: opts?.datePartition });
